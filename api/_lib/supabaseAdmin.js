...
function extractAdminUserPayload(data) {
  if (!data) {
    return null
  }
...
async function updateProfileRecord(updates) {
  try {
...
export async function getAdminUserById(userId) {
  ...
  const user = extractAdminUserPayload(data)
...
export async function updateAdminUser(userId, updates) {
  ...
  const currentUser = extractAdminUserPayload(currentData)
  ...
  await updateProfileRecord(profilePayload)
